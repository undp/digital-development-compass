...
async function main() {
  ...
  const scores = await csvtojson().fromFile(SCORES_FILE);
  const confidences = await csvtojson().fromFile(CONFIDENCES_FILE);
  const indices = await csvtojson().fromFile(INDICES_FILE);
  const sources = await csvtojson().fromFile(SOURCES_FILE);
  const latlon = require(LATLON_FILE);

  const pillarNames = [
    "Overall",
    ..._.uniq(scores.map((d) => d["Pillar"]).filter(Boolean)),
  ];
  ...
  function getPillarRank(name, pillar) {
    const allPillarScores = scores
      .filter((score) => score["Pillar"] === pillar && !score["Sub-Pillar"])
  ...
  function getSubpillarRank(name, pillar, subpillar) {
    const allPillarScores = scores
      .filter(
        (score) =>
          score["Pillar"] === pillar && score["Sub-Pillar"] === subpillar
      )
      .sort((a, b) => parseFloat(b["Score"]) - parseFloat(a["Score"]));
  ...
  function getPillarScore(country, pillar) {
    let match = scores.find((score) => {
      return (
        score["Country or Area"] === country &&
        score["Pillar"] === pillar &&
        !score["Sub-Pillar"]
      );
    });

    return match && match["Score"] ? match["Score"] : null;
  }
  ...
  function getPillarConfidence(country, pillar) {
    let match = confidences.find((confidence) => {
      return confidence["Country Name"] === country;
    });

    return match && match[pillar] ? match[pillar] : null;
  }
  ...
  function getSubpillarConfidence(country, subpillar) {
    ...
    return match && match[subpillar] ? match[subpillar] : null;
  }
  ...
  const getOverallConfidence = (country) => {
    const pillarConfidences = pillarNames
      .filter((p) => p !== "Overall")
      .map((pillar) => {
        let confidence = getPillarConfidence(country, pillar);
        return confidence ? parseFloat(confidence) : 0;
      });

    return _.mean(pillarConfidences);
  };
