...
export const ScoreRing = ({
  ...
  const numberOfMissingSubpillars = pillars.reduce((acc, pillar) => {
  ...
  pillars.forEach(([name, subpillars]) => {
    ...
    subpillars.forEach((subpillar, j) => {
      const isEmpty = !country.scores[name][subpillar].score;
  ...
  const arcTicks = range(0, 5, 1).map((i) => {
            ...
            return (
              ...
              >
                {range(0, 5).map((i) => {
                  ...
                  return (
                    <Fragment key={i}>
                      <stop offset={`${i * (100 / 5)}%`} stopColor={color} />
                      <stop
                        offset={`${(i + 1) * (100 / 5)}%`}
        ...
        <g transform={`translate(${width / 2}, ${height})`}>
          {pillars.map(([pillar, subpillars], index) =>
            subpillars.map((subpillar) => {
              ...
              const hasData = country.scores[pillar][subpillar].score;
              return (
                <g
                  key={`${pillar}-${subpillar}-${index}`}
                  style={{
                    opacity: isHovered || hasData ? 1 : 0.5,
                  }}
                ...
                >
                  <path
                    ...
                    strokeWidth={1}
                  />
                  <path d={fillArc} fill={`url(#${pillar}-gradient)`} />
                </g>
              );
            })
          )}
            ...
            return (
              <g key={`${pillar}-${index}`}>
                ...
                {subpillars.map((subpillar) => {
                  ...
                  // @ts-ignore
                  const value = country.scores[pillar][subpillar]?.["score"];
                  const hasData = !!value;
                  // @ts-ignore
...
const Info = ({
  ...
  return (
    <div className="absolute top-[73%] bottom-0 left-[36%] right-[36%]">
      <div className="">
        <h3
        ...
        {Boolean(stageInfo) ? (
          ...
          <div className="text-gray-500 mt-6 font-light italic whitespace-nowrap pointer-events-none">
            No data
          </div>
        )}
      </div>
